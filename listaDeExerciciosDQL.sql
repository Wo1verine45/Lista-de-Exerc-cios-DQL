...
Crie uma consulta que liste todos os alunos, ordenando-os primeiro por nome em ordem alfabética e, em caso de nomes iguais, pela data de nascimento (do mais velho 
para o mais novo).
*/
SELECT * 
FROM aluno 
...
*/
SELECT d.nome AS nome_disciplina, c.nome AS nome_curso, c.carga_horaria
FROM disciplina d
...
Obs: A coluna carga_horaria, teria que estar na tabela disciplina para essa pesquisa funcionar e fazer sentido
*/
SELECT c.nome AS curso, SUM(d.carga_horaria) AS carga_total
FROM curso c
...
INNER JOIN disciplina d ON c.id_curso = d.id_curso
GROUP BY c.nome
HAVING SUM(d.carga_horaria) > 2000; -- Não usar alias dentro da própria chamada, pode gerar erro, talvez seja só no HAVING
...
INNER JOIN turma t ON p.id_professor = t.id_professor
GROUP BY p.nome
ORDER BY COUNT(t.id_turma) DESC;
...
*/
SELECT a.nome AS aluno, m.status AS status, m.data_matricula AS data_matricula
FROM aluno a
INNER JOIN matricula m ON a.id_aluno = m.id_matricula
...
*/
SELECT a.nome AS aluno, TRUNC(MONTHS_BETWEEN(SYSDATE, a.data_nascimento) / 12) AS idade
FROM aluno a
ORDER BY idade DESC;
...
GROUP BY d.nome -- Não esquecer do GROUP BY ao usar funções agregadores com o COUNT 
...
*/
SELECT p.nome AS professores, d.nome AS disciplinas, t.horario AS horario
FROM turma t
INNER JOIN professor p ON t.id_professor = p.id_professor
INNER JOIN disciplina d ON t.id_disciplina = d.id_disciplina -- Quando for usar 2 JOINs, usar em linhas diferentes
...
*/
SELECT COUNT(*) AS disciplinas_mais_80
FROM disciplina 
...
*/
SELECT c.nome AS curso, COUNT(d.id_disciplina) AS total_discplinas
FROM curso c
INNER JOIN disciplina d ON c.id_curso = d.id_curso -- Atentar para ver se está comparando os campos certos
GROUP BY c.nome;
...
*/
SELECT p.nome AS professor, COUNT(t.id_disciplina) AS total_disciplinas_mais_100
FROM professor p
INNER JOIN turma t ON p.id_professor = t.id_professor
INNER JOIN disciplina d ON t.id_disciplina = d.id_disciplina -- Pode fazer os JOINs antes do WHERE
...
*/
SELECT d.nome AS disciplina, COUNT(m.id_aluno) AS alunos_matriculados
FROM disciplina d
INNER JOIN matricula m ON d.id_disciplina = m.id_disciplina
GROUP BY d.nome
...
*/
SELECT m.status AS status, COUNT(m.id_aluno) AS total_alunos
FROM matricula m
GROUP BY m.status
ORDER BY COUNT(m.id_aluno) DESC;
...
FROM professor p
INNER JOIN turma t ON p.id_professor = t.id_professor
INNER JOIN disciplina d ON t.id_disciplina = t.id_disciplina
GROUP BY p.nome
